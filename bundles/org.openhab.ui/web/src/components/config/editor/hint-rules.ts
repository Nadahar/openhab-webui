...
function isModuleElement(line: Line | undefined): boolean {
  ...
  return line.text.match(/^    (triggers|conditions|actions):\s*$/) ? true : false
}

function hintModuleType(context: CompletionContext, parentLine: Line | undefined): CompletionResult | Promise<CompletionResult | null> | null {
...
function hintModuleValue(context: CompletionContext, line: Line, parentLine: Line): CompletionResult | Promise<CompletionResult | null> | null {
  if (!line) return null
  const colonPos = line.text.indexOf(':')
...
export default function hint(context: CompletionContext): CompletionResult | Promise<CompletionResult | null> | null {
  ...
  const colonPos = line.text.indexOf(':')
  const afterColon = colonPos > 0 && cursor > colonPos
  const arrayElement = line.text.trimStart().startsWith('- ') || 
