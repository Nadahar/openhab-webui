        ...
        <f7-block>
          ...
          <div class="button-stack">
            ...
            <f7-button
              ...
              :tooltip="canCopyToYAML ? (showYamlExportOptions ? 'Hide YAML options' : 'Show YAML options') : (showYamlErrors ? 'Hide YAML errors' : 'Show YAML errors')"
    ...
    copyRuleDefinitionsToClipboard(type, serializationOption) {
      ...
      createFileFormatForRules(
        ...
        copyToClipboard(ruleDefinition, {
          ...
          onSuccess: () => {
            showToast(`${this.selectedItems.length} ${type || 'YAML'} rule definition${this.selectedItems.length === 1 ? '' : 's'} copied to clipboard`)
          },
          onError: () => {
            f7.dialog.alert(`Error copying rule ${type || 'YAML'} definition${this.selectedItems.length === 1 ? '' : 's'} to the clipboard`, 'Error')
          }
        })
